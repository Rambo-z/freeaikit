      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-5">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 bg-gray-100 rounded-xl p-1">
            {(["text-to-bin", "bin-to-text"] as Mode[]).map((m) => (
              <button
                ...
                className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all ${mode === m ? "bg-white text-indigo-700 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
              >
          ...
          <button
            onClick={swap}
            disabled={!output}
            className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold border border-gray-200 rounded-xl hover:border-indigo-400 hover:bg-indigo-50 disabled:opacity-40 transition-all"
          >
